...
export default function QuoteCard({ quote, onAccept, onDecline, decliningId, customerProfile, onCardSaved }) {
  ...
  useEffect(() => {
    if (!quote?.provider_id) return;
    base44.entities.ProviderProfile.filter({ id: quote.provider_id })
      .then(results => { if (results?.[0]) setProviderProfile(results[0]); })
      .catch(() => {});
  }, [quote?.provider_id]);
  const { t } = useLanguage();
          ...
          <div className="flex items-center gap-2 mb-1">
            ...
            <div>
              ...
              <div className="flex items-center gap-1 text-xs text-muted-foreground">
                ...
                <span>{providerProfile?.avg_rating ? Number(providerProfile.avg_rating).toFixed(1) : "New"}</span>
                <span>·</span>
                <span>{providerProfile?.total_jobs_completed ? `${providerProfile.total_jobs_completed} jobs` : "New provider"}</span>
              </div>
            </div>
          </div>
          {quote.message && (
