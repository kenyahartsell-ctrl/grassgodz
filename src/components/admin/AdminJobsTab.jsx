  ...
  return (
    <div className="bg-white border border-amber-200 rounded-xl p-4 shadow-sm">
      ...
      <div className="flex gap-2 flex-wrap">
        ...
        <button onClick={() => handlers.onCompleteNoPayment(job)} className="flex-1 py-1.5 text-xs font-semibold bg-teal-600 text-white rounded-lg hover:bg-teal-700 transition-colors" title="Complete (Skip Payment)">Complete (No Pay)</button>
      ...
      {showModal && (
        <AdminAssignProviderModal
          job={job}
          onClose={() => setShowModal(false)}
          onAssigned={() => { setShowModal(false); onAssigned && onAssigned(); toast.success('Job reassigned!'); }}
        />
      )}
      {showEditDate && <EditDateModal job={job} onClose={() => setShowEditDate(false)} onSaved={onRefresh} />}
    </div>
