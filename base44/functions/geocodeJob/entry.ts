...
async function geocodeAddress(address) {
  // Try MAPBOX_SECRET_TOKEN first, fall back to VITE_MAPBOX_SECRET
  const token = Deno.env.get('MAPBOX_SECRET_TOKEN') || Deno.env.get('VITE_MAPBOX_SECRET');
  ...
  if (data.message) throw new Error(`Mapbox error (${res.status}): ${data.message}`);
