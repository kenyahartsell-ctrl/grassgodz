...
import { Search, CheckCircle2, XCircle, Loader2, Shield, DollarSign, Banknote } from 'lucide-react';
...
export default function AdminCashOverridePanel({ jobs, providers, customers, onRefresh }) {
  ...
  const verifyProvider = async (job) => {
    if (!job.provider_id) {
      toast.error('No provider assigned to this job. Assign a provider first.');
      return;
    }
    setVerifying(true);
    ...
    try {
      ...
      // Provider's completed jobs with pending payout (not yet paid out / collateral)
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      // Also set allow_cash_payment on customer profile
    ...
    <div className="space-y-5">
      ...
      {approvedJobs.length > 0 && (
        <div className="bg-green-50 border border-green-200 rounded-xl p-4">
          <p className="text-xs font-bold text-green-800 mb-2">✓ {approvedJobs.length} job{approvedJobs.length !== 1 ? 's' : ''} with cash payment approved</p>
      ...
      {/* Jobs needing override */}
      <div className="space-y-3">
        {filteredJobs.length === 0 && (
          <div className="text-center py-10 text-sm text-muted-foreground">
            {search.length >= 2 ? 'No matching jobs found.' : 'No active jobs pending cash override.'}
          </div>
        )}
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground">
                      Provider: <strong>{job.provider_name || <span className="text-amber-600">Unassigned</span>}</strong>
                    </p>
                    {job.scheduled_date && (
                      <p className="text-xs text-muted-foreground">
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="grid grid-cols-2 gap-x-8 gap-y-1.5 text-xs mb-3">
                        ...
                        <div>
                          <p className="text-muted-foreground">Platform fee required (10%)</p>
                      ...
                      {!verificationResult.canApprove && (
                        <div className="bg-red-100 border border-red-200 rounded-lg px-3 py-2 mb-3 text-xs text-red-700">
